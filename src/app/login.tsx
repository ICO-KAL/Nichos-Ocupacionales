...
export default function LoginScreen() {
  ...
  return (
    <AuthScreen
      ...
      footer={
        <>
          <TextAction title="Olvide mi contrasena" onPress={() => router.push('/forgot-password')} />
          <TextAction title="Crear una cuenta" onPress={() => router.push('/register')} />
        </>
      }
      title="Bienvenido de nuevo">
      <FormField autoComplete="email" label="Correo electronico" onChangeText={setEmail} placeholder="tu@correo.com" value={email} />
      <FormField autoComplete="password" label="Contrasena" onChangeText={setPassword} placeholder="Tu contrasena" secureTextEntry value={password} />
      <ErrorMessage message={error} />
      <PrimaryButton isLoading={isLoading} onPress={handleLogin} title="Iniciar sesion" />
    </AuthScreen>
  );
}